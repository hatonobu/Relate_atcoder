...
N,M,sx,sy = mi()
dx = defaultdict(SortedMultiset)
dy = defaultdict(SortedMultiset)
...
for i in range(M):
    ...
    if command == "U":
        ...
        left_num = sx - num
        while(True):
            ret = dy[sy].ge(left_num)
            if  ret == None or ret > sx:
                break
            else:
                ans += 1
                dy[sy].discard(ret)
        ...
        left_num = sy - num
        while(True):
            ret = dx[sx].ge(left_num)
            if  ret == None or ret > sy:
                break
            else:
                ans += 1
                dy[ret].discard(sx)
